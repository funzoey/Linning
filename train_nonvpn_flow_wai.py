...
from Transformer import Transformer_wai, Transformer_pkt,classification
from tool_nonvpn import traffics_nonvpn,load_epoch_data_flow,max_byte_len
...
parser.add_argument('--num_pkt', type=int, default=6, help='Numbers of packet.')
...
parser.add_argument('--dim_model', type=int, default=120, help='Embedding Size.')
parser.add_argument('--d_ff', type=int, default=480, help='Hidden Size.')
...
# parser.add_argument("--load_path", type=str, default='./results/nonvpn/session/lessdetail/session2pkts_len_60_500_42_256')
parser.add_argument("--save_path", type=str, default='./results/nonvpn/session/lessdetail/session2flows_wai')
...
def test_epoch(model2, test_data):
...
def train_epoch(model2, training_data, optimizer):
	''' Epoch operation in training phase'''
	# model1.eval()
	model2.train()
...
def main2(i, flow_dict):
	...
	model2 = Transformer_wai(args).cuda()
	...
	for epoch_i in trange(args.num_epochs, mininterval=2, \
		...
		# print("train_x:", train_x.size, "train_y:", train_y.size, "train_label:", train_label.size)
		# print("train_x:", train_x.shape, "train_y:", train_y.shape, "train_label:", train_label.shape)
		...
		train_loss, train_acc = train_epoch(model2, training_data, optimizer)
		...
		test_acc, pred, test_time = test_epoch(model2, test_data)
...
if __name__ == '__main__':
	...
	for i in range(1):
		with open('./dataset/data_nonvpn/session/lessdetail/session2flows_len_6_60_500/nonvpn_flow_6_60_500_%d.pkl'%i, 'rb') as f:
