...
from o import traffics_app,load_epoch_data_flow,max_byte_len
...
parser.add_argument("--load_path", type=str, default='./results/app/session/lessdetail/flowpic_wth2_nonvpn_session2pkts_len_50_42_256')
parser.add_argument("--save_path", type=str, default='./results/app/session/lessdetail/flowpic_wth2_nonvpn_session2flows_len_6_50_4222_256')
		...
		# write F1, PRECISION, RECALL
			with open('%s/metric_%d.txt'%(args.save_path, i), 'w') as f3:
				...
				for a, b, c in zip(fscore, p, r):
					...
					f3.write('%.2f %.2f %.2f\n'%(a, b, c))
					f3.flush()
				if len(fscore) != len(traffics_app):
					a = set(pred)
					b = set(test_label[:,0])
					f3.write('%s\n%s'%(str(a), str(b)))
...
if __name__ == '__main__':
	...
	for i in range(1):
		with open('./dataset/data_app_nonvpn/session/lessdetail/wth2_nonvpn_flowpic_flow_6_50_500_%d.pkl'%i, 'rb') as f:
